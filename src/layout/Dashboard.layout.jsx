    ...
    <div className="w-full md:min-h-screen">
      {isAdmin && (
        <div className={classes.notifications__bar}>
          <div className="flex items-center">Logged in as client: {adminSession && adminSession.fullName}</div>
          <div>
            <Button
            ...
            {/* to={`${process.env.REACT_APP_ADMIN_SITE_URL}/admin/sign-in`} */}
          </div>
        </div>
      )}
      ...
      <div className="flex">
        ...
        <div className="flex flex-col col">
          <div className="bg-[#1A1A27] px-[20px] py-[20px] md:px-[60px] flex items-center gap-3">
            ...
            {activeSub?.name ? (
              <span className="text-[white] text-[10px]">{`${activeSub?.name} ${active?.name?.split("/").join("&")}`}</span>
            ) : ("")
            }
          </div>
          {children}
